# CH105 Kurogane Akari Gate A Review Manifest v001

> Backlog: `ART-1401`, `ART-1402`, `ART-1403`, `ART-1404`, `ART-1409`
> Cross-review support: `ART-1501`, `ART-1502`
> Status: `APPROVED / 2D PRODUCTION SHEET`
> `APPROVED`: **GRANTED**

## Source lock

- Primary source: `art_refs/characters/roster/current/CH101-105_Roster_FaceDiversity_WIP_v003.png`
- Source commit: `418ef96` — `art: refine current roster gate A visuals`
- Identity source: fifth figure from the left only, CH105 Kurogane Akari
- Canonical contracts: `docs/05_art_concept/CHARACTER_BIBLE.md`, `docs/05_art_concept/CHARACTER_ANCHOR_SPEC.md`
- Roster source status: WIP / conditional review; it does not grant individual Gate A approval

## Current package

```text
art_refs/characters/akari/concept/CH105_Akari_GateA_Candidates_WIP_v001.png
art_refs/characters/akari/concept/CH105_Akari_GateA_Candidates_WIP_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_GateA_Candidates_WIP_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_Face128_WIP_A_v001.png
art_refs/characters/akari/concept/CH105_Akari_Face128_WIP_A_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_Face128_WIP_A_v001.metadata.md
art_refs/characters/roster/current/CH101-105_Roster_Face128_REVIEW_v001.png
art_refs/characters/roster/current/CH101-105_Roster_Face128_REVIEW_v001.metadata.md
art_refs/characters/roster/current/CH101-105_Roster_Face128_REVIEW_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_Silhouette_ON_OFF_WIP_A_v001.png
art_refs/characters/akari/concept/CH105_Akari_Silhouette_ON_OFF_WIP_A_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_Silhouette_ON_OFF_WIP_A_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_REVIEW_v001.png
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_REVIEW_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_REVIEW_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_APPROVED_v001.png
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_APPROVED_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_CharacterSheet_APPROVED_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_ExpressionSheet_REVIEW_v001.png
art_refs/characters/akari/concept/CH105_Akari_ExpressionSheet_REVIEW_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_ExpressionSheet_REVIEW_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_EquipmentSheet_REVIEW_v001.png
art_refs/characters/akari/concept/CH105_Akari_EquipmentSheet_REVIEW_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_EquipmentSheet_REVIEW_v001.metadata.md
art_refs/characters/akari/concept/CH105_Akari_MaterialPalette_REVIEW_v001.png
art_refs/characters/akari/concept/CH105_Akari_MaterialPalette_REVIEW_v001.review.md
art_refs/characters/akari/concept/CH105_Akari_MaterialPalette_REVIEW_v001.metadata.md
art_source/manifests/CH105_Akari_2DTo3D_Translation_REVIEW_v001.md
art_source/manifests/CH105_Akari_GateA_Review_Request_20260812.md
art_source/manifests/CH105_Akari_GateB_Preflight_REVIEW_v001.md
art_source/manifests/CH105_Akari_Production_Handoff_REVIEW_v001.md
```

## Identity and state contract

| Axis | Locked WIP anchor |
|---|---|
| Face | mature amber eyes, stronger brows, stable adult feminine face read |
| Hair | black outer mass with teal inner/back hair |
| Costume | black and deep forest-green utility layers with copper-gold hardware |
| Body read | broad-shouldered, grounded frontline protector; not full armor |
| Equipment | one heavy gauntlet, one anchor ring, restrained cable line |
| State use | A Neutral, B Ready, C Full Guard are one identity package |

## Gate A evidence status

| Check | Result | Evidence |
|---|---|---|
| Morning source fidelity | `PASS FOR WIP` | Fifth roster figure is used as the only identity source |
| A/B/C shared identity | `PASS FOR WIP` | Candidate board and source invariants |
| Face identity support | `PASS FOR GATE B INPUT` | Individual face board plus current-roster exact 128px comparison board; five-person verdict recorded as PASS |
| Equipment ON/OFF | `PASS FOR GATE B INPUT` | ON/OFF/LOW-SAT board and five-person cross-review support the approved 2D direction; technical proof remains open |
| Unified character sheet | `PASS` | Front/side/back plus A/B/C state strip is approved as the current 2D production reference |
| Expression sheet | `PASS FOR REVIEW` | Six restrained identity-preserving expressions are assembled; deformation proof remains open |
| Equipment sheet | `PASS FOR REVIEW` | Single ring, heavy gauntlet, A/B/C states, and 2D simplification direction are assembled; technical proof remains open |
| Color/material sheet | `PASS FOR REVIEW` | Source-faithful palette roles, material separation, and low-saturation value hierarchy are assembled; shader proof remains open |
| 2D→3D translation | `DOCUMENTED FOR REVIEW` | Keep/enlarge/simplify/remove rules, state presentation, provisional sockets, material categories, and no-physics policy are recorded; technical proof remains open |
| 5-person separation | `PASS` | Current-roster Face128 and ON/OFF/LOW-SAT evidence were reviewed; the cross-roster decision is recorded as PASS |
| 2D→3D proof | `NOT TESTED` | No mesh, blockout, socket, rig, shader, or Unity proof |
| Human Gate A | `APPROVED` | User approved the CH105 2D package on 2026-08-12; see the separate production-sheet approval record |

## Cross-review impact

- `ART-1501`: CH105 now has a current-roster-faithful approved 2D package; the 5/5 roster verdict is recorded as PASS.
- `ART-1502`: CH105 now has an exact 128px current-roster comparison board plus ON/OFF/LOW-SAT support; the 5/5 verdict is recorded as PASS while the board remains REVIEW evidence.
- Current cross-review package: `art_refs/characters/lineup/ReCamp_CurrentRoster_ART-1501-1504_REVIEW_Package_20260812.md`
- The earlier 2026-08-07 lineup package is not evidence for this roster; it covers the retired Luna/Miyu/Coco/Iris/Noah set.

## Approval boundary

This manifest records the approved CH105 2D production reference. It does not approve Gate B, 3D Blockout,
Blender/FBX, Unity import, Runtime roster replacement, Android proof, or release use.

## Next action

- Approval record: `art_source/manifests/CH105_Akari_Production_Sheet_Approval_20260812.md`

1. Keep the approved CH105 2D sheet linked to the current CH101–CH104 approved sheets.
2. Prepare a separate CH105 Gate B preflight from this translation contract.
3. Use `CH105_Akari_GateB_Preflight_REVIEW_v001.md` and `CH105_Akari_Production_Handoff_REVIEW_v001.md` as documentation-side Gate B inputs; keep 3D, Unity, Runtime, Android, and release work blocked until their separate prerequisites and approvals exist.
