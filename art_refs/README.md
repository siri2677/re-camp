...
→ 원본 전투 콘셉트 PRIMARY REVIEW
→ APPROVED 아트 디렉션 보드
→ REVIEW 라인업
→ WIP
```

현재 사람 승인된 최종 캐릭터 시트는 없다. `REVIEW` 이미지는 제작 방향 비교용이며 출시 자산이나 3D 모델링 시트로 직접 사용하지 않는다.
...
- 성인 여성 5인, 남성향 서브컬처, 2D 일반 등신, 3D 4~5등신 SD-lite 방향을 유지한다.
