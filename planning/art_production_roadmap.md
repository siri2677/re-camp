...
- `ART_DIRECTION.md` v2.1
- `CHARACTER_BIBLE.md` v2.1
- `CHARACTER_ANCHOR_SPEC.md` v2.1
- `ART_REVIEW_CHECKLIST.md` v2.1
- 성인 여성 5인·남성향 캐릭터 구성
...
- 5인 도구 비종속 캐릭터 앵커·회귀 규칙 v2.1
- 캐릭터·환경·UI/VFX Gate A/B/C 공통 리뷰 체크리스트 v2.1
...
## A2. 5인 2D 디자인 재탐색·방향 확정 — Review

일본 모바일 서브컬처 화풍으로 5인 후보·선택 KeyArt·제작 근거를 다시 작성했고, 개별 제작 시트는
루나 → 미유 → 코코 → 이리스 → 노아 순으로 `REVIEW` 패키지에 묶었다. 과거 서양식 세미리얼 얼굴과
페인터리 렌더링 결과는 `STYLE REJECTED / HISTORY ONLY`이며 현행 얼굴·렌더링 앵커로 재사용하지 않는다.
ComfyUI 도입을 기다리지 않고 현재 이미지 도구와 결정론적 합성 도구로 검토 근거를 완성했다.
...
- 필수 이미지·비율·128px 얼굴·5인 교차 비교 근거: `REVIEW` 작성 완료
- 일본 모바일 서브컬처 얼굴·헤어·셀/소프트 그라데이션 계약: 문서와 현행 이미지에 적용
- 사람 Gate A 매력도·최종 방향 승인: `PENDING`
- `APPROVED`: `NOT GRANTED`

따라서 계획한 제작 작업은 사람 검토가 가능한 단계까지 완료됐지만, A2의 공식 완료 조건은 사용자가
5인 방향을 직접 승인한 뒤에만 충족한다.

### 2026-08-09 사용자 레퍼런스 품질 매트릭스

사용자가 지정한 승리의 여신: 니케, 이환, 젠레스 존 제로, 블루 아카이브, 히간: 이루실은 성인 여성의 매력·여성성·표정·패션·자세·마감도를 분해하는 품질 레퍼런스로만 사용한다. 외부 작품의 고유 캐릭터·의상·무기·포즈·UI·로고는 제작 앵커로 사용하지 않는다.

현재 5인 REVIEW 패키지는 ART-1510에서 다음을 재검토한다.

- 얼굴·체형·패션·자세에서 성인 여성의 매력과 개별 매력 축이 즉시 전달되는가
- 3초 첫인상·128px 얼굴·장비 제거 실루엣·저채도에서 5인이 구분되는가
- 레퍼런스 품질 축이 Re:Camp의 폐허 탐험·캠프·직업 장비로 변형됐는가
- 사람 Gate A 승인 전 상태를 REVIEW로 유지하고, 승인된 루나만 A3에 진입하는가

상세 계획은 planning/DESIGN_DIRECTION_2026-08-09.md를 따른다.
...
- 이리스: 2D 7.2~7.4 → 3D 5.7~5.8, 화이트 롱 코트·비총기형 에너지 관측 랜스·바이저
