...
상세 기준은 `docs/00_project/CURRENT_PROJECT_BASELINE.md`를 따른다.
...
5. 별도 유료 도구 없이 제작·검증이 가능한가
6. Vertical Slice 승인 후 안전하게 확장 가능한가
...
- Windows 우선, Android 후속으로 플랫폼 통일
...
- Aura AI를 유료 의존 도구에서 제외
- Unity 자동화를 Coplay + Editor Script·Preset + 수동 검증으로 통일

### 완료 기준

- `CURRENT_PROJECT_BASELINE.md`와 계획 문서가 충돌하지 않음
- Draft PR이 리뷰되고 병합됨
- 신규 작업이 새 ID와 무료 중심 도구 기준을 사용함
...
Unity 프로젝트와 아트 제작 도구를 준비해 Gray Box와 루나 제작을 시작할 수 있게 한다.
...
- `ART-PIPE-0107` Inspector·Console·Prefab Diff·PlayMode 검증 Report
- `ART-PIPE-0108` Git LFS와 `art_source/` 구조

### 완료 기준

```text
Unity 프로젝트 Clone 후 실행
+ Camera Gray Box
+ Core Adapter 방식
+ ComfyUI 반복 생성
+ Figma Template
...
Gray Box가 완주되지 않으면 나머지 네 캐릭터 프로토타입과 확장 기능을 중단하고 루나 핵심 루프에 집중한다.
...
- `ART-3D-2006` Import·Prefab Validation Tool과 수동 QA
- `ART-3D-2007` 고정 쿼터뷰 실루엣 테스트

### 완료 기준

- 루나의 얼굴·후드·단검이 쿼터뷰에서 식별됨
- Unity Humanoid Avatar 생성
- Prefab 구조와 Import 규칙을 재현 가능
- Console Error·Missing Reference 없음
- 최종 모델링 전 수정할 비율·장식·무기 크기 확정
