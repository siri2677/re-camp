...
제품·플랫폼·전역 비주얼·로스터·현재 Gate는 `docs/00_project/CURRENT_PROJECT_BASELINE.md`를 따른다.
...
기존 5인 v002 KeyArt와 파생 시트는 2026-08-07 사용자 검토에서 화풍 불일치와 목표 품질 미달로
Gate A `FAIL` 판정을 받았다. 현재 핵심은 동일 렌더링 파이프라인의 5인 얼굴/흉상 스타일 마스터를
먼저 승인하고, 그 공통 참조로 5인 전신·개별 KeyArt·파생 시트를 다시 만드는 것이다.
...
| M1 | 아트 방향·5인 설정 v2.1 | In Progress | 문서 계약은 존재하지만 v002 시각 패키지 Gate A 실패, 하우스 스타일 재설정 중 |
...
| M4 | 아트 파이프라인·5인 Approved 2D 시트 | In Progress | 기존 개별·교차 시트는 역사 자료, 얼굴 스타일 락부터 재제작 필요 |
...
- 5인 v002 KeyArt·후보·3뷰·표정·장비·팔레트·2D→3D·통합 비교의 실패·기술 이력
- 동일 얼굴·홍채·선화·피부·헤어 렌더링을 시험하는 5인 얼굴·전신 스타일 캘리브레이션 WIP

### 남은 작업

- 5인 얼굴 스타일 마스터 수정과 사용자 Gate A 재판정
- 승인된 공통 스타일로 5인 전신·KeyArt·파생 시트 재생성
...
기존 5인 `REVIEW` 패키지와 결정론적 통합 시트는 화풍 불일치로 Gate A에 실패했다. 파일은 기술 이력으로
보존하고, 얼굴 스타일 마스터 승인 후 동일 공통 참조로 재생성한다. ComfyUI는 대량 일관성·정밀 인페인팅이
실제 병목일 때만 `Deferred`에서 전환하며, 사람 승인 전 어떤 이미지도 최종 게임 자산으로 승격하지 않는다.
