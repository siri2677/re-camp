...
2026-08-09까지 기록된 최신 직접/자동 검증 기준:
...
이 문서 단일화 작업에서는 Unity를 다시 실행하지 않았다. 위 수치는 마지막 실제 검증 기록을 보존한 것이다.
...
테스트 수는 테스트가 추가·삭제되거나 직접 실행됐을 때만 이 문서에서 갱신한다.

## 9. 아직 증명되지 않은 것

- Current branch의 완전한 Fresh Clone·Unity Open·Build·Test parity.
- Hosted CI의 Current branch 기준 성공 결과.
