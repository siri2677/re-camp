...
Secondary Scope: 3D 5-to-6-head stylized translation notes only
Runtime Asset: No
```

- 템플릿 상태 `REVIEW`는 레이아웃의 사용성 검토 상태다.
- 이 템플릿으로 만든 캐릭터 시트가 자동으로 `APPROVED`되는 것은 아니다.
- 2D 프레임에는 일반 등신만 배치한다. 3D 스타일라이즈드 이미지는 별도 3D 참고 프레임에서만 사용한다.
...
| 05 | `Palette_Materials_3DNotes` | 대표/보조/포인트색, 재질, 5~6등신 변환 주의사항 |
...
- 2D 일반 등신의 어떤 특징을 3D 5~6등신에서 확대·단순화할지
- 기본 목표 약 5.2~5.5등신, 머리·손·발·대표 장비 크기, 관절·Rig·관통 위험
