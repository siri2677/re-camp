...
Current Status: WIP / REVIEW / CONDITIONAL / APPROVED
Anchor Spec Version: 3.0
Character Bible Version: 3.0
Input References:
Tool / Model:
License Evidence:
...
Gate A는 PNG 후보를 사람이 비교해 제작 방향을 선택하는 단계다. Unity Import·3D 모델·최종
Animation은 입력이 아니다. 모든 후보는 `REVIEW` 상태와 companion metadata를 가져야 한다.

#### 입력 조건 고정

- 모든 후보를 같은 캔버스·인물 크기·카메라 높이·조명·배경 밀도로 배치한다.
- 첫 번째 관찰에서는 이름·ID·설명·후보 번호를 숨겨 선입견을 줄인다.
- 캐릭터별로 전신 3/4 PNG, 128px 얼굴, 장비 포함/제거 실루엣, 팔레트, metadata를 준비한다.
- 후보 간 차이는 `pose`, `expression`, `fabric`, `background light` 범위로 제한한다. 얼굴·헤어·장비
  구조가 달라진 후보는 같은 캐릭터의 변형이 아니라 별도 방향으로 기록한다.

#### 고정 검토 순서

1. **3초 첫인상** — 이미지를 3초만 보여주고 가린다. 관찰자가 적은 훅·역할·성인성 단어가
   Character Bible과 맞는지 기록한다. 이름을 보지 않고도 대표 장비와 성격이 설명돼야 한다.
2. **전신·얼굴** — 머리 단위로 2D 비율을 확인하고, 목표 범위 약 6.8~7.4등신과 캐릭터별
   목표값을 기록한다. 128px에서 눈매·헤어·대표색·감정이 남는지 확인한다.
3. **역할·장비** — 대표 장비만 보고 전투 역할과 탐험·캠프 기능을 추측한다. 장비가 장식으로만
   보이거나 Bible의 역할과 다른 경우 `FAIL`이다.
4. **실루엣** — 장비 포함 흑색 실루엣과 장비 제거 흑색 실루엣을 같은 크기로 비교한다. 얼굴·헤어·
   체형·대표 장비 중 최소 두 개가 즉시 구분돼야 한다.
5. **저채도·색각 보정** — 색을 제거하거나 채도를 낮춘 뒤 형태·장비·표정만으로 구분한다.
   색상만 사라져 구분되지 않으면 `FAIL` 또는 수정 조건을 기록한다.
6. **성격·표정** — 평상시와 임무 상태의 표정에서 캐릭터의 반전 매력을 확인한다. 기능복과
   무기만 남고 성격이 읽히지 않으면 `CONDITIONAL` 이하로 판정한다.
7. **원작성** — 사용자 레퍼런스와 나란히 놓고 흡수한 품질 축과 차이점을 분리 기록한다.
   고유 얼굴·의상 절개·무기·포즈·로고를 추적 가능하게 복제하면 즉시 `FAIL`이다.
8. **2D→3D 가능성** — 5~6등신으로 줄일 때 얼굴·헤어·대표 장비·기능 실루엣이 남는지 확인한다.
   가는 케이블·긴 천·부유 파츠·얼굴 가림은 삭제·통합 후보로 기록하며 아직 3D 승인을 의미하지 않는다.

#### 5인 교차 검토

- 다섯 후보를 같은 축척으로 한 화면에 배치하고 얼굴 스트립, 장비 포함/제거 실루엣, 저채도 보드를
  별도로 만든다.
- 각 캐릭터의 훅·대표 장비·대표색·체형·헤어·성격 단어를 행으로 두고, 다른 캐릭터와 충돌하는
  요소를 직접 표시한다.
- 한 캐릭터가 개별 검토를 통과해도 라인업에서 다른 캐릭터의 전용 모티프를 침범하면 `CONDITIONAL`
  또는 `FAIL`로 낮춘다.

#### 증거와 판정 기록

- `CHA-A01`~`CHA-A19` 각 항목에 `PASS`, `CONDITIONAL`, `FAIL`, `NOT TESTED`와 한 줄 근거를 기록한다.
- 전신·128px·실루엣·저채도·색각 보정·레퍼런스 차이 보드를 review evidence로 연결한다.
- Reviewer, Date, Decision, Next Owner, 수정 요청과 재검토 범위를 반드시 기록한다.
- 하나라도 `FAIL`이면 Gate 전체는 `FAIL`이다. `CONDITIONAL` 또는 `NOT TESTED`가 있으면
  `APPROVED`로 승격하지 않는다.
- 모든 필수 항목이 `PASS`이고 사람이 명시적으로 승인한 경우에만 `APPROVED`로 바꾼다.
...
| CHA-A03 | 큰 보석형 눈·짧은 중안부·작은 코와 입·애니메이션 헤어·깨끗한 셀 색면을 갖춘 일본 서브컬처형 얼굴이며, 체형·의상 패션까지 남성향 대표 캐릭터로 충분한 매력을 가짐 | 얼굴 크롭·전신·스타일 앵커 비교 | |
...
- 아마사와 린이 일반 닌자·쌍단검 사용자, 키사라기 마오가 장총·드론 사용자,
  카나데 노조미가 간호사 코스튬·대형 방패 사용자, 시온 타테가미가 일반 총기·랜스 사용자,
  쿠로가네 아카리가 얼굴과 허리선이 사라진 중장갑 남성 탱커로 회귀
