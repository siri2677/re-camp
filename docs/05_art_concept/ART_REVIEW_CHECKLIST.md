...
| CHA-A17 | 5인이 동일 게임의 동일 아트 디렉션으로 보이며 선화·홍채·피부 그림자·헤어 하이라이트·광원·재질 렌더링 규칙이 일치 | 동일 조건 얼굴/흉상 스타일 마스터와 전신 라인업 | |
| CHA-A18 | 성인 여성의 짧은 하의·허벅지 노출이 캐릭터별 패션·체형·역할에 맞게 유지되며, 긴 바지 일괄 적용이나 장식 스트랩 반복으로 사용자 매력 방향을 훼손하지 않음 | 5인 전신 라인업·노출 면적 비교 | |
| CHA-A19 | 루나는 치마형 힙 패널 없이 차콜 하이웨이스트 쇼츠를 사용하고, 미유는 쇼츠가 보이는 짧은 라일락 비대칭 랩 패널을 사용하되 바닥 케이블이 없음 | 루나·미유 하체 확대 비교 | |
...
- 캐릭터별 선화·얼굴 해부·피부 명암·헤어 채색·광원·배경이 달라 서로 다른 작가 또는 게임처럼 보임
