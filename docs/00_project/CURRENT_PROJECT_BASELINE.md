...
수익은 플랫폼 우선순위를 결정하지 않는다. 등록비·외주비·구독비를 감당할 수 있는지만 확인한다.

- Google Play 등록과 테스트 요건은 출시 준비 시 재확인한다.
- 무료 출시, 저가 유료 앱, 무료 체험 후 1회 해금 중 어떤 방식이 적합한지는 Vertical Slice 외부 테스트 후 결정한다.
- 가챠·광고·유료 재화를 전제로 게임 구조를 변경하지 않는다.
- 손익분기 판매량은 필수 목표가 아니라 과도한 외주·구독 지출을 방지하는 참고값이다.
- 수익이 낮아도 완성도 높은 공개 빌드와 포트폴리오 결과를 성공으로 인정한다.
