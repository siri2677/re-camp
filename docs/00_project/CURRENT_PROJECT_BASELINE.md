...
| Gate A/B/C 공통 판정 | `ART_REVIEW_CHECKLIST.md` v2.0 완료 |
...
새 제작은 v003을 교정하는 방식이 아니라 Character Bible·Anchor v2.0의 한 문장 훅을 기준으로
캐릭터별 약 7등신 후보를 처음부터 재탐색한다. 사람 승인 전 `Direction Approved`나 `APPROVED`로 기록하지 않는다.
