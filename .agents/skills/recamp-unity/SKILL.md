...
2. Use `rg` in `planning/sprint_backlog.md` and `planning/IMPLEMENTATION_STATUS.md` to locate only the current `P0`, `Ready`, or `In Progress` item and its completion criteria.
3. Read other planning, art, or docs files only when the selected work item references them. Open `planning/COPLAY_MCP_HANDOFF.md` only for connection, stale-job, or regression recovery.
4. Inspect `git status --short` before editing and preserve all unrelated user changes.
...
5. Restore `Assets/_ReCamp/Scenes/03_Battle.unity` as the active scene unless the task intentionally changes the handoff scene.
